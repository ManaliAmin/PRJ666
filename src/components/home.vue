        ...
        <div slot="body">
          <div class="modal-dialog">
            <div class="loginmodal-container">
              <input type="button" name="login" @click="goTo('/TextModule')" id = "buttonBtn" class="login loginmodal-button" value="View Courses">
              <input type="button" name="login" @click="goTo('/CreateCourse')" id = "buttonBtn" class="login loginmodal-button" value="Create Course">
              <input type="button" name="login" @click="goTo('/present')" id = "present" class="login loginmodal-button" value="Present Course">
            </div>
          </div>
        </div>
      <!-- </div> -->
			<!-- <div v-show="showOneButton">
        <h1 slot="header"><div style="display: inline">Welcome to knowledgeflow</div>
        <div style="display: inline; float:right; font-size: 20px; cursor:pointer;" @click="closeModal"><span aria-hidden="true">&times;</span></div></h1>
        <div slot="body">
          <div class="modal-dialog">
            <div class="loginmodal-container">
              <input type="button" name="login" @click="signin" id = "buttonBtn" class="login loginmodal-button" value="Login">
            </div>
          </div>
        </div>
			</div> -->
		</modal>
    <!-- <modal ref:modal3 v-show="modal3">
			<h1 slot="header"><div style="display: inline">Welcome to knowledgeflow</div>
      <div style="display: inline; float:right; font-size: 20px; cursor:pointer;" @click="closeModal"><span aria-hidden="true">&times;</span></div></h1>
			<div slot="body">
        <div class="modal-dialog">
  ...
  methods:{
    ...
    signin: function (event) {
      ...
      for(var i=0; i < vm.users.length; i++){
        if(this.username.toLowerCase() == vm.users[i].name.toLowerCase() && this.password == vm.users[i].password){
          if(this.username.toLowerCase() == "colin"){
            this.goTo("/admin");
          } else {
            vm.users[i].login_counter = 5;
            if(vm.users[i].login_counter == 0){
              this.modal1 = false;
              this.modal2 = true;
            ...
            // console.log(this.showOneButton);
            // console.log(this.showThreeButtons);
            // console.log("counter", vm.users[i].login_counter);
            
          } 
          vm.updateSigninCounter(this.username);
        }else {
